...
import { ReverseItNumeric } from '../services/DataService';

export default function AddingComponent() {
  const [ num, setNum ] = useState(0);
  const [ result, setResult ] = useState('');

  const reverseItNumeric = async () => {
      const response = await ReverseItNumeric(num);
      setResult(response);
  }

  return (
    <div>
      <header className='reverseItNumBg'>
      <div className='center-items' style={{marginLeft:"5px", marginRight:"5px"}}>
               <h1 className=' pt-5  text-white'>Reverse It Numeric</h1>
                </div>
                ...
                <div className='center-items mt-5'>
                    <Button className='mt-5 oddBtn' onClick={ () => reverseItNumeric() } style={{ width: "100px",borderRadius:"100px", overflow: 'hidden'}}> <h5 className='mb-0'>Click</h5></Button>
                </div>
